...
parameters = c(
   ...
   beta.S4_SP = 0.0002,              # Subadults, Year 4
   ...
   m.S1_SP = log(1-.339)/(-365),     # Subadults, Year 1
   m.S2_SP = log(1-.326)/(-365),     # Subadults, Year 2
   m.S3_SP = log(1-.270)/(-365),     # Subadults, Year 3
   m.S4_SP = log(1-.270)/(-365),     # Subadults, Year 4
   m.A1_SP = log(1-.268)/(-365),     # Adults, Year 1
   m.A2_SP = log(1-.268)/(-365),     # Adults, Year 2
   # Rate of ageing
   ...
   mu.L_SP = 0,                      # Larvae
   mu.J_SP = log(1-.309)/(-365),     # Juveniles
   mu.S1_SP = log(1-.309)/(-365),    # Subadults, Year 1
   mu.S2_SP = log(1-.309)/(-365),    # Subadults, Year 2
   mu.S3_SP = log(1-.310)/(-365),    # Subadults, Year 3
   mu.S4_SP = log(1-.310)/(-365),    # Subadults, Year 4
   mu.A1_SP = log(1-.310)/(-365),    # Adults, Year 1
   mu.A2_SP = log(1-.310)/(-365),    # Adults, Year 2
   ...
   gamma.S4_SP = log(1-.956)/(-365), # Subadults, Year 4
   ...
   lambda.S4_SP = 0.1,               # Subadults, Year 4
   ...
   beta.S3_LE = 0.0002,              # Subadults, Year 3
   beta.A1_LE = 0.0002,              # Adults, Year 1
   beta.A2_LE = 0.0002,              # Adults, Year 2
   # Natural mortality rates
   m.L_LE = log(0.052)/(-365),     # Larvae
   m.J_LE = log(0.052)/(-365),      # Juveniles
   m.S1_LE = log(1-.203)/(-365),     # Subadults, Year 1
   m.S2_LE = log(1-.188)/(-365),     # Subadults, Year 2
   m.S3_LE = log(1-.120)/(-365),     # Subadults, Year 3
   m.A1_LE = log(1-.118)/(-365),     # Adults, Year 1
   m.A2_LE = log(1-.118)/(-365),     # Adults, Year 2
   # Rate of ageing
   ...
   # Added mortality rate due to infection
   mu.L_LE = 0,                      # Larvae
   mu.J_LE = log(1-.671)/(-365),     # Juveniles 
   mu.S1_LE = log(1-.671)/(-365),    # Subadults, Year 1
   mu.S2_LE = log(1-.682)/(-365),    # Subadults, Year 2
   mu.S3_LE = log(1-.730)/(-365),    # Subadults, Year 3
   mu.A1_LE = log(1-.731)/(-365),    # Adults, Year 1
   mu.A2_LE = log(1-.731)/(-365),    # Adults, Year 2
   # recovery rate from infection
   gamma.L_LE = 0,                   # Larvae
   gamma.J_LE = log(1-.949)/(-365),  # Juveniles 
   gamma.S1_LE = log(1-.949)/(-365), # Subadults, Year 1
   gamma.S2_LE = log(1-.949)/(-365), # Subadults, Year 2
   gamma.S3_LE = log(1-.949)/(-365), # Subadults, Year 3
   ...
   lambda.S3_LE = 0.1,               # Subadults, Year 3
...
growth <- function(parms) {   
  ...
  GrowthSP <- exp(mean(c(
                      ...
                      summary(lm(log(results$S_S4.1_SP)~xvals))$coefficients[2,1],
  ...
  GrowthLE <- exp(mean(c(
                      ...
                      summary(lm(log(results$S_S3.1_LE)~xvals))$coefficients[2,1],
...
findRootSP(0, 5, 1.094038,0.00001)
# L. spenceri bisection exploring instantaneous daily growth rates from 0 to 5
findRootLE(0, 5, 2.214612,0.00001)
