...
load("LHS_high.RData") # load Latin Hypercube sample of parameter sets
...
fail_filter <- function(vector, LE_SP, pMin, pMax, yEQ){
...
for (j in 1:nrow(parameter_sets)){    # for each LHS parameter set
  ...
  source(file = 'High_quasi.R')       # simulate to quasi-eq
  initial_conditions[[j]] <- init.out # store quasi-eq states across compartment
...
save(parameter_sets, filter_vals, fail, initial_conditions, year_of_quasiEQ, adult_densities, file = 'High_quasi.RData')
