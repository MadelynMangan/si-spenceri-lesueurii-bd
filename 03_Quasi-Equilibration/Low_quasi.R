# File: Low_quasi.R
# Title: Quasi-equilibration of a low elevation SI model for spenceri/lesueurii
...
#     This script is called by the script Low_quasi_set.R, which cycles through a parameter set of
#     length N and runs each set to its quasi-equilibrium. The model runs year-by-year until quasi-eq
#     is reached. Quasi-equilibrium is satisfied when both species exhibited an annual change in adult 
#	  population size ≤1% for 20 consecutive years. To account for the special case where one species 
#	  moving indefinitely towards extinction does not meet this criterion (i.e. population densities < 1), 
#	  quasi-equilibrium is also assumed when the adult ratio between species surpassed 1:100. After 
#	  quasi-eq is reached, the file outputs the following:

#         (1) winter and summer prevalence of infection in adults of each species during the year of 
#             quasi-eq
#         (2) the LE-to-SP adult ratio in the year of quasi-eq
#         (3) the adult density of each species for every year of the simulation
#         (4) the final density for every compartment in the model (to be used as initial values in
#             subsequent simulations from quasi-eq
#     
#     Each population starts with the same population density and AGE structure, which is 
#     then dispersed among life stages based on their population STAGE structure. (Because 
#     L. lesueurii has an expedited life cycle, this species therefore has more density in its final 
#     adult stage). Running to quasi-equilibrium will move past transient dynamics that arise as a
#	  result of this arbitrary age structure. 
#     
#     Management Modifiers: certain parameters contain modifiers which scale many parameters at once. 
#     These modifiers are all set to 1 (no change) when running to equilibrium, but will be used to
#     simulate management scenarios in subsequent scripts. 
...
initial <- c(
   ...
   # Adults, Year 1
   S_A1.1_SP = n3*z.SP, # Susceptible
   I_A1.1_SP = 0,       # Infected 

   #Adults, Year 2
   S_A2_SP = (n4+n5+n6)*z.SP, # Susceptible 
   I_A2_SP = 0,               # Infected 
   ...
   # Adults, Year 1
   S_A1.1_LE = n2*z.LE, # Susceptible
   I_A1.1_LE = 0,       # Infected 

   # Adults, Year 2
   S_A2_LE = (n3+n4+n5+n6)*z.LE,  # Susceptible 
   I_A2_LE = 0,                   # Infected 
...
parameters = c(
   ...
   l.S2_SP = 1/365,       # Subadults, Year 2 to Adults, Year 1
   ...
   # Transmission rates from zoospore pool to host
   beta.L_LE = set$beta.L_LE*beta_mod,    # Larvae
   beta.J_LE = set$beta.J_LE*beta_mod,    # Juveniles
   beta.S1_LE = set$beta.S1_LE*beta_mod,  # Subadults, Year 1
   beta.A1_LE = set$beta.A1_LE*beta_mod,  # Adults, Year 1
   beta.A2_LE = set$beta.A2_LE*beta_mod,  # Adults, Year 2
   # Natural mortality rates
   ...
   l.S1_LE = 1/365,       # Subadults, Year 1 to Adults, Year 1
...
SIS_2host <- function (t, x, params) {
   ...
   # Differential equations, with variables as defined above
      ...
      dS_A1.1_SP.dt = l.S2_SP*S_S2.1_SP-beta.A1_SP*S_A1.1_SP*Z - m.A1_SP*S_A1.1_SP + gamma.A1_SP*I_A1.1_SP - l.A1_SP*S_A1.1_SP              
      dI_A1.1_SP.dt = l.S2_SP*I_S2.1_SP+beta.A1_SP*S_A1.1_SP*Z - m.A1_SP*I_A1.1_SP - mu.A1_SP*I_A1.1_SP - gamma.A1_SP*I_A1.1_SP - l.A1_SP*I_A1.1_SP    
      # Adults, Year 2
      ...
      # Adult, Year 1, Substage 1
      dS_A1.1_LE.dt = l.S1_LE*S_S1.1_LE-beta.A1_LE*S_A1.1_LE*Z - m.A1_LE*S_A1.1_LE + gamma.A1_LE*I_A1.1_LE - l.A1_LE*S_A1.1_LE              
      dI_A1.1_LE.dt = l.S1_LE*I_S1.1_LE+beta.A1_LE*S_A1.1_LE*Z - m.A1_LE*I_A1.1_LE - mu.A1_LE*I_A1.1_LE - gamma.A1_LE*I_A1.1_LE - l.A1_LE*I_A1.1_LE    
      # Adults, Year 2
...
densSP <- n3+n4+n5+n6    # extract initial SP density
densLE <- n2+n3+n4+n5+n6 # extract initial LE density
...
ratio_LE_SP <- with(results[end,], A_LE_Total/A_SP_Total) # LE-to-SP adult ratio
prevalence_SP <- with(results[end,], (I_A1.1_SP + I_A2_SP) / A_SP_Total) # SP winter adult prevalence
prevalence_SP5 <- with(results[end-1,], (I_A1.1_SP + I_A2_SP) / A_SP_Total) # SP summer adult prevalence
prevalence_LE <- with(results[end,], (I_A1.1_LE + I_A2_LE) / A_LE_Total) # LE winter adult prevalence
prevalence_LE5 <- with(results[end-1,], (I_A1.1_LE + I_A2_LE) / A_LE_Total) # SP summer adult prevalence

# compile filter values into vector
filters <- c("ratio" = ratio_LE_SP, "prev_SP" = prevalence_SP, "prev_SP5" = prevalence_SP5, 
             "prev_LE" = prevalence_LE, 'prev_LE5' = prevalence_LE5)

# extract quasi-eq densities from all compartments for downstream management scenarios
init.out <- unlist(results[end,2:index])
