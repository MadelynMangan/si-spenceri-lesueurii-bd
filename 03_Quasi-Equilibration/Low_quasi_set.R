...
load("LHS_low.RData") # load Latin Hypercube sample of parameter sets
...
for (j in 1:nrow(parameter_sets)){    # for each LHS parameter set
  ...
  source(file = 'Low_quasi.R')        # simulate to quasi-eq
...
save(parameter_sets, filter_vals, fail, initial_conditions, year_of_quasiEQ, adult_densities, file = 'Low_quasi.RData')
